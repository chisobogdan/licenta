...
import com.java.licenta2018.util.Util;
...
public class ZoneObserver extends Observer {
	...
	public void update() {
		Util util = new Util();
		String mesaj = util.createMessageForView(subject.getMessage(),
				subject.getQueueName());

		msgView.setZona(subject.getZona());
		msgView.setMessage(mesaj);
		msgView.setContor(msgView.getContor() + 1);

		// System.out.println(subject.getQueueName() + " " +
		// subject.getMessage() + " " + subject.getZona());
	}
